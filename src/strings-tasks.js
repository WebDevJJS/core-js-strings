...
function removeFirstOccurrences(str, value) {
  ...
  return idx > -1 ? str.replace(value, '') : str;
}
...
function countVowels(str) {
  return str.length - str.replaceAll(/[aeiouy]/gi, '').length;
}
...
function findLongestWord(sentence) {
  return sentence.split(/\W/).sort((a, b) => b.length - a.length)[0];
}
...
function invertCase(str) {
  return str.replace(/\w/g, (c) => {
    ...
    return c === lower ? c.toUpperCase() : c.toLowerCase();
  });
}
...
function extractNameFromTemplate(value) {
  return value.replace(/Hello, (\w+ \w+)!/, '$1');
}
...
function encodeToRot13(str) {
  return str.replace(/\w/g, (char) =>
    String.fromCharCode(
      char.charCodeAt(0) + (char.toLocaleLowerCase() > 'm' ? -13 : 13)
    )
  );
}
